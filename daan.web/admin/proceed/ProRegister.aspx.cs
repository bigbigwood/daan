...
{
    public partial class ProRegister : PageBase
    {
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            ...
            if (barcodeservice.CheckBarCode(tbxbarcode.Text.Trim()))//条码号存在
            {
                MessageBoxShow(string.Format("此条码号[{0}]已在本系统内生成，请更改条码号！", this.tbxbarcode.Text.Trim()));
                return;
            }
        ...
        private void BindDropdownList(ExtAspNet.DropDownList dp,List<Dicttestitem> list)
        {
            ...
            {
                myList.Add(new CustomClass(item.Dicttestitemid,string.Format("{0}[{1}]",item.Testname,item.Testcode)));
            }
